...
- (void)viewDidLoad {
    ...
    _arrayTitle = @[@"活动标题",@"球场",@"活动人数",@"人均价格",@"联系人",@"联系人电话",@"活动开始日期",@"活动开始时间",@"活动结束日期",@"活动内容"];
...
-(void)bianjiClick
{
    ...
    else
    {
        ...
        [_dict setObject:_teamId forKey:@"team_Id"];
        if (![Helper isBlankString:_strBall]) {
            [_dict setObject:_strBall forKey:@"ballName"];
        }
...
-(void)uiConfig
{
    _tableView = [[UITableView alloc]initWithFrame:CGRectMake(0, 0, ScreenWidth, 10*44*ScreenWidth/375) style:UITableViewStylePlain];
...
//            _strTime
        ...
        {
            cell.detroLabel.text = @"请输入球队详情内容";
        }
...
-(BOOL)textField:(UITextField *)textField shouldChangeCharactersInRange:(NSRange)range replacementString:(NSString *)string{
    ...
    if (textField.tag == 1231 || textField.tag == 1234) {
...
-(BOOL)textFieldShouldEndEditing:(UITextField *)textField
{
    ...
    switch (textField.tag - 1230) {
        case 0:
            //标题
            [_dict setObject:textField.text forKey:@"teamActivityTitle"];
            break;
        case 1:
            //人数
            
            [_dict setObject:textField.text forKey:@"peopleNums"];
            break;
        case 2:
                //联系人
            [_dict setObject:textField.text forKey:@"contactperson"];
           
            break;
        case 3:
            //电话
            if (![Helper testMobileIsTrue:textField.text]) {
                textField.text = @"";
                textField.placeholder = @"请输入正确的手机号";
                [self.view endEditing:YES];
                return NO;
            }
            else
            {
                [_dict setObject:textField.text forKey:@"contactphone"];
            }
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    switch (indexPath.row){
        ...
        {
            //简介
            ActivityIntroViewController* introVc = [[ActivityIntroViewController alloc]init];
//            [introVc introBlock:^(NSString *introStr) {
            introVc.introBlock = ^(NSString* strIntro){
                _strIntro = strIntro;
                [_dict setValue:strIntro forKey:@"activityContent"];
                NSIndexPath *indexPath_1=[NSIndexPath indexPathForRow:indexPath.row inSection:0];
                NSArray *indexArray=[NSArray arrayWithObject:indexPath_1];
                [_tableView reloadRowsAtIndexPaths:indexArray withRowAnimation:UITableViewRowAnimationAutomatic];
            };
