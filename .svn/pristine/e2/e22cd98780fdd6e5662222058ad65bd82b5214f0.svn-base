...
@interface TeamIntroCViewController ()<UITextViewDelegate>
{
...
@implementation TeamIntroCViewController
...
-(void)finishClick
{
    if (![Helper isBlankString:_textView.text]) {
        [[PostDataRequest sharedInstance]postDataRequest:@"team/update.do" parameter:@{@"teamId":_teamId,@"teamSign":_textView.text} success:^(id respondsData) {
            ...
            {
                UIAlertView *alert = [[UIAlertView alloc]initWithTitle:@"提示" message:[dict objectForKey:@"message"] delegate:nil cancelButtonTitle:@"确定" otherButtonTitles:nil];
                [alert show];
            }
    ...
    {
        UIAlertView *alert = [[UIAlertView alloc]initWithTitle:@"提示" message:@"您还没有填写球队简介" delegate:nil cancelButtonTitle:@"确定" otherButtonTitles:nil];
        [alert show];
    }
