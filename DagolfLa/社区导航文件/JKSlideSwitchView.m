...
@implementation JKSlideSwitchView
...
static const CGFloat kBottomLingHeight = 3.0f;
//static const CGFloat kSpeaceWither = 3.0f;
...
- (void)initValues
{
    ...
    self.topView = [[UIImageView alloc]initWithFrame:CGRectMake(0, 0, screenWidth, 75 * screenWidth/375)];
    self.topView.image = [UIImage imageNamed:@"nav"];
    self.topView.userInteractionEnabled = YES;
    [self addSubview:self.topView];
    
    //创建顶部可滑动的tab
    _topScrollView = [[UIScrollView alloc] initWithFrame:CGRectMake(ScreenWidth/6*1, 20*screenWidth/375,ScreenWidth/6*4, kHeightOfTopScrollView)];
    ...
    //创建主滚动视图
    _rootScrollView = [[UIScrollView alloc] initWithFrame:CGRectMake(0, _topScrollView.frame.origin.y + _topScrollView.frame.size.height, self.bounds.size.width, self.bounds.size.height - kHeightOfTopScrollView - 20*screenWidth/375)];
...
{
    _scrollViewSelectedChannelID = index + 100;
    UIButton *btn = (UIButton *)[_topScrollView viewWithTag:_scrollViewSelectedChannelID];
...
- (void)createNameButtons
{
    ...
    _lineLabel.backgroundColor = _bottomLineColor;
...
//        [button setBackgroundColor:[UIColor blueColor]];
        ...
        CGSize textSize = [vc.title boundingRectWithSize:CGSizeMake(100, 0) options: NSStringDrawingTruncatesLastVisibleLine | NSStringDrawingUsesLineFragmentOrigin | NSStringDrawingUsesFontLeading attributes:attribute context:nil].size;
        
        //累计每个tab文字的长度
        topScrollViewContentWidth += kWidthOfButtonMargin + textSize.width;
        //设置按钮尺寸
        [button setFrame:CGRectMake(xOffset,0,
                                    _topScrollView.frame.size.width/_viewArray.count, kHeightOfTopScrollView)];
        //计算下一个tab的x偏移量
        xOffset += _topScrollView.frame.size.width/_viewArray.count + kWidthOfButtonMargin;
        
        [button setTag:i+100];
        if (i == 0) {
            _lineLabel.frame = CGRectMake(kWidthOfButtonMargin, 40, _topScrollView.frame.size.width/_viewArray.count, kBottomLingHeight);
            button.selected = YES;
        }
        [button setTitle:vc.title forState:UIControlStateNormal];
        button.titleLabel.font = [UIFont systemFontOfSize:kFontSizeOfTabButton *ScreenWidth/375];
...
- (void)selectNameButton:(UIButton *)sender
{
    ...
    if (!sender.selected) {
        ...
        [UIView animateWithDuration:0.05 animations:^{
            [_lineLabel setFrame:CGRectMake(sender.frame.origin.x, 40, sender.frame.size.width , kBottomLingHeight)];
...
- (void)selectFirst{
    UIButton *lastButton = (UIButton *)[_topScrollView viewWithTag:100];
