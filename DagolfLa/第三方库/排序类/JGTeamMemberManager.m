...
+(NSArray *)archiveNumbers:(NSArray *)originalArray{
    ...
    for(int i=0;i<[stringsToSort count];i++){
        ...
        JGLTeamMemberModel *model = stringsToSort[i];
    ...
    for (int i = 0; i < chineseStringsArray.count; i++) {
        BOOL addd = YES;
        JGLTeamMemberModel *model = chineseStringsArray[i];
...
+(NSArray *)activityArchiveNumbers:(NSArray *)originalArray{
    ...
    for(int i=0;i<[stringsToSort count];i++){
        ...
        JGHPlayersModel *model = stringsToSort[i];
        //JGHPlayersModel
        model.chineseString = [[ChineseString alloc]init];
        model.chineseString.string = model.name;
