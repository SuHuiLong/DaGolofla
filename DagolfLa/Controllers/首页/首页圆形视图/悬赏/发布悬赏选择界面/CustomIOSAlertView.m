...
- (UIView *)createContainerView
{
    if (containerView == NULL) {
        containerView = [[UIView alloc] initWithFrame:CGRectMake(0, 0, 300*ScreenWidth/375, 150*ScreenWidth/375)];
    }
...
- (CGSize)countScreenSize
{
    ...
    CGFloat screWidth = [UIScreen mainScreen].bounds.size.width;
    CGFloat screHeight = [UIScreen mainScreen].bounds.size.height;

    // On iOS7, screen width and height doesn't automatically follow orientation
    if (floor(NSFoundationVersionNumber) <= NSFoundationVersionNumber_iOS_7_1) {
        UIInterfaceOrientation interfaceOrientation = [[UIApplication sharedApplication] statusBarOrientation];
        if (UIInterfaceOrientationIsLandscape(interfaceOrientation)) {
            CGFloat tmp = screWidth;
            screWidth = screHeight;
            screHeight = tmp;
        }
    }
...
- (void)changeOrientationForIOS8: (NSNotification *)notification {
    ...
    [UIView animateWithDuration:0.2f delay:0.0 options:UIViewAnimationOptionTransitionNone
                     animations:^{
                         ...
                         self.frame = CGRectMake(0, 0, screWidth, screHeight);
                         dialogView.frame = CGRectMake((screWidth - dialogSize.width) / 2, (screHeight - keyboardSize.height - dialogSize.height) / 2, dialogSize.width, dialogSize.height);
                     }
                     completion:nil
     ];
