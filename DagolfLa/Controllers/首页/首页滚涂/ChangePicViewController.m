...
@interface ChangePicViewController ()<UIWebViewDelegate, WCLPassWordViewDelegate, JGHbalanceViewDelegate>
...
- (void)viewDidLoad {
    ...
    _dictCan = [NSMutableDictionary dictionary];
...
- (BOOL)webView:(UIWebView *)webView shouldStartLoadWithRequest:(NSURLRequest *)request navigationType:(UIWebViewNavigationType)navigationType
{
    ...
    if ([str containsString:@"createTeam"]) {
    ...
    if ([str containsString:@"dagolfla://"]) {
        ...
        if ([str containsString:@"teamWithDraw"]) {
            if ([str containsString:@"?"]) {
                JGDWithDrawTeamMoneyViewController *vc = [[JGDWithDrawTeamMoneyViewController alloc] init];
                vc.teamKey = [NSNumber numberWithInteger:[[Helper returnKeyVlaueWithUrlString:str andKey:@"teamKey"] integerValue]];
                [self.navigationController pushViewController:vc animated:YES];        }
        }
        
        // 球队大厅
        if ([str containsString:@"teamHall"]) {
        ...
        if ([str containsString:@"teamMemberMgr"]) {
            JGTeamMemberController* menVc = [[JGTeamMemberController alloc]init];
            menVc.title = @"队员管理";
            menVc.power = @"1004,1001,1002,1005";
            menVc.teamManagement = 1;
            menVc.teamKey = [NSNumber numberWithInteger:[[Helper returnKeyVlaueWithUrlString:str andKey:@"teamKey"] integerValue]];
            [self.navigationController pushViewController:menVc animated:YES];
        }
        
        // 入队审核页面
        
        if ([str containsString:@"auditTeamMember"]) {
            JGLJoinManageViewController *jgJoinVC = [[JGLJoinManageViewController alloc] init];
            jgJoinVC.teamKey = [NSNumber numberWithInteger:[[Helper returnKeyVlaueWithUrlString:str andKey:@"teamKey"] integerValue]];
            [self.navigationController pushViewController:jgJoinVC animated:YES];
        }
        
        //新球友
        if ([str containsString:@"newUserFriendList"]) {
        ...
        //活动成绩详情 --
        if ([str containsString:@"activityScore"]) {
            JGDActSelfHistoryScoreViewController *teamGroupCtrl= [[JGDActSelfHistoryScoreViewController alloc]init];
            teamGroupCtrl.teamKey = [NSNumber numberWithInteger:[[Helper returnKeyVlaueWithUrlString:str andKey:@"teamKey"] integerValue]];
            teamGroupCtrl.timeKey = [Helper returnKeyVlaueWithUrlString:str andKey:@"activityKey"];
            teamGroupCtrl.hidesBottomBarWhenPushed = YES;
            [self.navigationController pushViewController:teamGroupCtrl animated:YES];
        }
        
        //获奖详情 --
        if ([str containsString:@"awardedInfo"]) {
            JGLPresentAwardViewController *teamGroupCtrl= [[JGLPresentAwardViewController alloc]init];
            teamGroupCtrl.activityKey = [[Helper returnKeyVlaueWithUrlString:str andKey:@"activityKey"] integerValue];
            teamGroupCtrl.teamKey = [[Helper returnKeyVlaueWithUrlString:str andKey:@"teamKey"] integerValue];
        ...
        //球队详情
        if ([str containsString:@"teamDetail"]) {
            JGDNewTeamDetailViewController *newTeamVC = [[JGDNewTeamDetailViewController alloc] init];
            newTeamVC.timeKey = [NSNumber numberWithInteger:[[Helper returnKeyVlaueWithUrlString:str andKey:@"timekey"] integerValue]];
            newTeamVC.hidesBottomBarWhenPushed = YES;
            [self.navigationController pushViewController:newTeamVC animated:YES];
        }
        
        //商品详情
        if ([str containsString:@"goodDetail"]) {
            UseMallViewController* userVc = [[UseMallViewController alloc]init];
            userVc.linkUrl = [NSString stringWithFormat:@"http://www.dagolfla.com/app/ProductDetails.html?proid=%td", [[Helper returnKeyVlaueWithUrlString:str andKey:@"timekey"] integerValue]];
            userVc.hidesBottomBarWhenPushed = YES;
            [self.navigationController pushViewController:userVc animated:YES];
        }
        
        //H5
        if ([str containsString:@"openURL"]) {
            JGLPushDetailsViewController* puVc = [[JGLPushDetailsViewController alloc]init];
            puVc.strUrl = [Helper returnKeyVlaueWithUrlString:str andKey:@"timekey"];
            puVc.hidesBottomBarWhenPushed = YES;
            [self.navigationController pushViewController:puVc animated:YES];
        }
        
        //社区
        if ([str containsString:@"moodKey"]) {
            DetailViewController * comDevc = [[DetailViewController alloc]init];
            comDevc.detailId = [NSNumber numberWithInteger:[[Helper returnKeyVlaueWithUrlString:str andKey:@"timekey"] integerValue]];
