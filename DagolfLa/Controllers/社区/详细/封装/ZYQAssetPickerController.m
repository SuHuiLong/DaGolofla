...
+ (void)initialize        //AssetsPickerChecked
{

    checkedIcon     = [UIImage imageNamed:@"gou_x"];
...
//    [_imageView setImage:[UIImage imageWithCGImage:[[asset defaultRepresentation] fullScreenImage]]];
...
//        weakself.imageView.image = [weakself.imageV]
//        [weakself.imageView setImage:[UIImage im]]
//        [weakself.imageView setImage:[UIImage imageCroppedToFitSize:(CGSize){150,150}]];
        [weakself compressImage:_imageView.image];
        dispatch_async(dispatch_get_main_queue(), ^{
            //完成，设置到view
//            [weakself.imageView setImage:weakself.image];
//            [weakself.imageView setImage:[UIImage]]
            [_imageView setImage:weakself.imageView.image];
        });
    });
...
- (id)initWithNumber:(NSInteger)maxNumber;
@end
...
- (BOOL)shouldSelectAsset:(ALAsset *)asset
{
    if (_indexPathsForSelectedItems.count>_maxNumber-1) {
        NSString *string=[NSString stringWithFormat:@"您最多选择%ld张",(long)_maxNumber];
...
{
    ZYQAssetViewController *vc = [[ZYQAssetViewController alloc] initWithNumber:_maxNumber];
    vc.assetsGroup = [self.groups objectAtIndex:indexPath.row];
    ////NSLog(@"%@",vc.assetsGroup);
...
- (id)initWithNSinter:(NSInteger )maxNumber
{
    ZYQAssetGroupViewController *groupViewController = [[ZYQAssetGroupViewController alloc] initWithNumber:maxNumber];
    if (self = [super initWithRootViewController:groupViewController])
    {
        
        _maximumNumberOfSelection      = _maxNumber;
