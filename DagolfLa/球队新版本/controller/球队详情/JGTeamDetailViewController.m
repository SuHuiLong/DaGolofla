...
- (void)viewDidLoad {
    ...
    [teamDetaliV.applyJoin addTarget:self action:@selector(apply) forControlEvents:(UIControlEventTouchUpInside)];
    for (int i = 0; i < 2; i ++) {
        UIButton *button = [teamDetaliV.buttonBackView viewWithTag:200 + i];
    ...
    [teamDetaliV.topBackImageV addSubview:backBtn];
