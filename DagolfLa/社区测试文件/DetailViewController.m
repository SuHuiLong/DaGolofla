...
//    self.navigationController.navigationBarHidden = YES;
    self.title = @"评论详情";
//    self.title = [NSString stringWithFormat:@"%@",self.detailId];
...
- (void) initTableview{
    ...
    self.mainTable = [[UITableView alloc] initWithFrame:CGRectMake(0, 0, self.view.frame.size.width,ScreenHeight - 65)];
...
- (void)downLoadData:(int)page isReshing:(BOOL)isReshing{
    ...
    [dic setValue:_detailId forKey:@"id"];
    
    [[PostDataRequest sharedInstance] postDataRequest:@"userMood/get.do" parameter:dic success:^(id respondsData) {
...
//            for (NSDictionary *dataDict in [dict objectForKey:@"rows"]) {
                ...
                NSDictionary *dataDict = [dict objectForKey:@"rows"];
...
- (CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    return TableHeader + kLocationToBottom + (unfold?ym.shuoshuoHeight:ym.unFoldShuoHeight) + ym.showImageHeight + 75 + ym.replyHeight  +  (([ym.moodType intValue] == 0)?0:30) +  (ym.favourHeight == 0?0:20*screenWidth/375) + ([ym.forwardNum intValue] == 0?0:20*screenWidth/375) +(ym.islessLimit?0:30*screenWidth/375) - (distance ? 30 : 0) *screenWidth/375;
}
...
- (void)replyMessage:(YMButton *)sender indexPath:(NSInteger)indexPath{
    ...
    if ([[NSUserDefaults standardUserDefaults]objectForKey:@"userId"]) {
        ...
        replyView = [[YMReplyInputView alloc] initWithFrame:CGRectMake(0, self.view.frame.size.height, screenWidth,0) andAboveView:self.view];
