...
- (void) composeView
{
	...
	textView = [[UITextView alloc] initWithFrame:CGRectMake(70.0f*screenWidth/375, topGap, 185*screenWidth/375 + screenWidth - 320*screenWidth/375, 0)];
    textView.backgroundColor = [UIColor clearColor];
	textView.delegate = self;
    textView.contentInset = UIEdgeInsetsMake(-4*screenWidth/375, -2*screenWidth/375, -4*screenWidth/375, 0);
    textView.showsVerticalScrollIndicator = NO;
    textView.showsHorizontalScrollIndicator = NO;
    textView.returnKeyType = UIReturnKeySend;
	textView.font = [UIFont systemFontOfSize:15.0f*screenWidth/375];
	[self addSubview:textView];
    
    [self adjustTextInputHeightForText:@"" animated:NO];
    
    lblPlaceholder = [[UILabel alloc] initWithFrame:CGRectMake(78.0f*screenWidth/375, topGap+2*screenWidth/375, 160*screenWidth/375, 20*screenWidth/375)];
    lblPlaceholder.font = [UIFont systemFontOfSize:15.0f*screenWidth/375];
    ...
    sendButton.titleLabel.font = [UIFont systemFontOfSize:15.0f*screenWidth/375];
...
- (void)layoutSubviews
{
  ...
  //最上面的一条细线
    ...
    textView.frame = CGRectMake(5, textView.frame.origin.y, screenWidth - 10*screenWidth/375 - 65*screenWidth/375, textView.frame.size.height);
    CGRect f = textView.frame;
    f.size.height = f.size.height+3;
    textViewBackgroundView.frame = f;
    lblPlaceholder.frame = CGRectMake(8, topGap+2, 230*screenWidth/375, 20*screenWidth/375);
    lblPlaceholder.backgroundColor = [UIColor clearColor];
    sendButton.frame = CGRectMake(screenWidth - 10 - 55,textView.frame.origin.y, 55*screenWidth/375, 27*screenWidth/375);
}
...
- (void) adjustTextInputHeightForText:(NSString*)text animated:(BOOL)animated
{
    ...
    int h2 = [text sizeForFont:textView.font constrainedToSize:CGSizeMake(textView.frame.size.width - 16*screenWidth/375, 170.0f*screenWidth/375) lineBreakMode:NSLineBreakByWordWrapping].height;
