...
- (NSAttributedString *)createAttributedEmotionStringWithRanges:(NSArray *)ranges
    ...
    if (_textColor == nil) {
        ...
        _textColor = [JKSlideSwitchView colorFromHexRGB:@"32b14d"];
    }
...
- (int)getTextLines{
    ...
    while (start < length){
        ...
        y -= FontSize + LineSpacing;
        CFRelease(line);
//        //NSLog(@"count = %ld,start = %ld, string = %@",count,start, _attrEmotionString.string);
        textlines ++;
//        if (start != length) {
//            start = start - 3;
//        }
