...
+ (NSURL *)setImageIconUrl:(NSNumber *)timeKey {
    //        NSString *imageStr = [NSString stringWithFormat:@"http://192.168.2.18:8080/%@",downloadImageUrl];
    NSString *imageStr = [NSString stringWithFormat:@"http://imgcache.dagolfla.com/team/%@.jpg@100w_100h",timeKey];
    NSURL *imageUrl = [NSURL URLWithString:imageStr];
    return imageUrl;
}
...
+ (NSURL *)setImageIconUrl:(NSString *)iconType andTeamKey:(NSInteger)timeKey andIsSetWidth:(BOOL)isSet andIsBackGround:(BOOL)isBack
{
    ...
    else
    {
        ...
        {
            imageStr = [NSString stringWithFormat:@"http://imgcache.dagolfla.com/%@/%td_backgroup.jpg@400w_150h",iconType,timeKey];
        }
    }
