...
//    ////NSLog(@"12---==== %ld",firstComponents.weekday-1);
...
//    _indexWeek = firstWeekday;
//    ////NSLog(@"123 ====  %ld",_indexWeek);
    ...
    if (indexPath.row < firstWeekday) {    // 小于这个月的第一天
        ...
        if (day == [[NSCalendar currentCalendar] component:NSCalendarUnitDay fromDate:self.date]) {
            cell.backgroundColor = [UIColor colorWithRed:0.36f green:0.75f blue:0.42f alpha:1.00f];
            cell.layer.cornerRadius = cell.frame.size.height / 2;
            cell.dayLabel.textColor = [UIColor whiteColor];
            cell.chineseDayLabel.textColor = [UIColor whiteColor];
//            ////NSLog(@"cell indedx == %ld",indexPath.row);
            NSNumber* index = [NSNumber numberWithInteger:indexPath.row];
            [_array addObject:index];
        ...
        if ([[NSCalendar currentCalendar] isDate:[NSDate date] equalToDate:self.date toUnitGranularity:NSCalendarUnitMonth] && ![[NSCalendar currentCalendar] isDateInToday:self.date]) {
            ...
            if (day == [[NSCalendar currentCalendar] component:NSCalendarUnitDay fromDate:[NSDate date]]) {
                cell.dayLabel.textColor = [UIColor whiteColor];
                cell.layer.cornerRadius = cell.frame.size.height / 2;
                cell.backgroundColor = [UIColor colorWithRed:0.65f green:0.65f blue:0.65f alpha:1.00f];
//                ////NSLog(@"cell indedx == %ld",indexPath.row);
            }
        }
...
//        ////NSLog(@"selexcted == %@",selectedDate)
    }
    
//    _callback(Weekdays[indexPath.row%7]);
    _day = Weekdays[indexPath.row%7];
